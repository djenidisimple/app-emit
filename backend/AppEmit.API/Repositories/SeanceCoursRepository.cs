...
namespace AppEmit.API.Repositories
{
    public class SeanceCoursRepository : ISeanceCoursRepository
    {
        ...
        public SeanceCoursRepository(AppDbContext context)
        {
            _context = context;
        }

        public async Task<List<SeanceCours>> GetSeancesForWeekAsync(
            DateTime monday, DateTime saturday,
            int? professeurId, int? salleId)
        {
            var query = _context.SeancesCours   // ← correction ici (pluriel)
                .Include(s => s.Matiere)
            ...
            if (professeurId.HasValue)
                query = query.Where(s => s.ProfesseurId == professeurId.Value);
            if (salleId.HasValue)
